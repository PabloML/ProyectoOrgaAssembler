...
section .data
  ...
  ayuda db "Programa para volcar el contenido de un archivo en formato hexadecimal y ASCII.", 10,
  ...
  lineal equ $ - linea
  buffer_ceros db "000000"			;Buffer para escribir la ultima vez la cantidad de elementos leidos
  buffer_cerosl equ $ - buffer_ceros
...
_start:
	...
	je 	tErrorOtro	; Se pasa a enumerar lo ingresado por pantalla.
...
unArg:
	...
	jne	open_file		; Si no hay un '-' solo puede haber un path  de archivo.
...
.continuar								
	...
	jg tErrorOtro ; Si no es -h significa que el parametro no es válido.
...
imprimir_contador:
	...
	mov EAX,[contador]
	mov EBX,buffer_ceros		;Buffer especial que contiene "000000"
	call caracter_contador

	mov EAX, 4		;SYS_WRITE
	mov EBX, 1 		;STDOUT
	mov ECX,buffer_ceros
	mov EDX,buffer_cerosl
	int 80h
	call imprimir_salto
	...
	pop EBX
	mov EAX, 6 		;SYS_CLOSE
	int 80h			; No se contempla el error al cerrar 
...
; Funcion convertir: Convierte el caracter que se encuentra en el registro DL 
; a hexa en ascii (0..9A..F)
; @Param:
;   DL - Caracter
; @Return:
